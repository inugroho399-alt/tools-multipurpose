...
export default function ConvertToJpgClient() {
      ...
      canvas.toBlob(
        (blob) => {
          if (blob) {
            ...
            setError("Gagal menghasilkan gambar JPG.");
            setState("error");
          }
        },
        "image/jpeg",
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = file.name.replace(/\.[^/.]+$/, "") + "-converted.jpg";
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      <div className="text-center mb-10">
        <div
          ...
          style={{ background: "linear-gradient(135deg,#eab308,#f97316)", boxShadow: "0 8px 30px rgba(234,179,8,0.35)" }}
        >
      ...
      <div className="flex justify-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium text-slate-400 bg-white/5 border border-white/8">
          <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse-slow"></span>
        ...
        <section className="mb-6 animate-fade-in">
          ...
          <div className="mb-6 p-4 rounded-xl bg-white/[0.02] border border-white/10">
            <label className="block text-sm font-medium text-slate-300 mb-4">
              Kualitas JPG: {Math.round(quality * 100)}%
            </label>
            <input
              type="range"
              min="10"
          ...
          {state !== "done" && (
            <div className="flex justify-center">
              <button
                ...
                style={{ background: "linear-gradient(135deg,#eab308,#f97316)", boxShadow: "0 4px 20px rgba(234,179,8,0.4)" }}
              >
                {state === "processing" ? "Sedang memproses..." : "Konversi ke JPG"}
              </button>
            </div>
          )}
        </section>
